...
function authServiceErrorMessage(error: unknown) {
  if (error instanceof Error && error.message.includes('Missing required public environment')) {
    return 'Supabase is not configured for this deployment. Add NEXT_PUBLIC_SUPABASE_URL and NEXT_PUBLIC_SUPABASE_ANON_KEY, then redeploy.';
  }

  if (error instanceof Error && error.message.includes('fetch failed')) {
    return 'Could not reach Supabase. Check that NEXT_PUBLIC_SUPABASE_URL points to your real Supabase project URL, not example.supabase.co.';
  }

  return 'Authentication service is temporarily unavailable. Please try again.';
}

async function createAuthClientOrRedirect(pathname: string, params: Record<string, string> = {}) {
  try {
    return await createSupabaseServerClient();
  } catch (error) {
    authRedirect(pathname, { ...params, error: authServiceErrorMessage(error) });
    throw new Error('Unreachable auth configuration redirect.');
  }
}

async function runAuthCallOrRedirect<T>(
  call: Promise<T>,
  pathname: string,
  ...
  try {
    ...
    authRedirect(pathname, { ...params, error: authServiceErrorMessage(error) });
    throw new Error('Unreachable auth service redirect.');
  }
}
...
export async function signInAction(formData: FormData) {
  ...
  const supabase = await createAuthClientOrRedirect('/login', { next });
  const { data, error } = await runAuthCallOrRedirect(
...
export async function signUpAction(formData: FormData) {
  ...
  const supabase = await createAuthClientOrRedirect('/signup');
  const { data, error } = await runAuthCallOrRedirect(
